...
if (isset($_POST['customer_app_mode']) || isset($_POST['seller_app_mode']) || isset($_POST['delivery_boy_app_mode']) || isset($_POST['website_mode'])) {
    if (defined('ALLOW_MODIFICATION') && ALLOW_MODIFICATION == 0) {
        echo '<label class="alert alert-danger">This operation is not allowed in demo panel!.</label>';
        return false;
    }
...
if (isset($_POST['customer_scripts']) && $_POST['customer_scripts'] == 1) {


    $header_script = isset($_POST['header_script']) && !empty($_POST['header_script']) ? $db->escapeString($_POST['header_script']) : "";


    $footer_script = isset($_POST['footer_script']) && !empty($_POST['footer_script']) ? $db->escapeString($_POST['footer_script']) : "";

    // print_r($message);

    $sql = "select * from settings where variable = 'header_script'";
    $db->sql($sql);
    $header_res = $db->getResult();


    $sql = "select * from settings where variable = 'footer_script'";
    $db->sql($sql);
    $footer_res = $db->getResult();
    $message = '';

    if (empty($header_res)) {
        $sql = "INSERT INTO settings (`variable`,`value`) VALUES ('header_script','$header_script')";
        // print_r($sql);
        if ($db->sql($sql)) {
        ...
        // $header_insert = $db->getResult();

    } else {
        // print_R($header_script);
        $sql = "UPDATE settings SET value='" . $header_script . "' WHERE variable='header_script'";
        if ($db->sql($sql)) {
        ...
        // $header_update = $db->getResult();



    }
    if (empty($footer_res)) {
        $sql = "INSERT INTO settings (`variable`,`value`) VALUES ('footer_script','$footer_script')";
        if ($db->sql($sql)) {
            echo "<p class='alert alert-success'>saved successfully</p>";
        ...
        // $footer_insert = $db->getResult();


    } else {

        $sql = "UPDATE settings SET value='" . $footer_script . "' WHERE variable='footer_script'";
        if ($db->sql($sql)) {
            echo "<p class='alert alert-success'>Update successfully</p>";
...
if (isset($_POST['front_end_settings']) && $_POST['front_end_settings'] == 1) {
    ...
    // $_POST['header_script'] = str_replace("'", "''", $_POST['header_script']);
    // $_POST['footer_script'] = str_replace("'", "''", $_POST['footer_script']);
...
if (isset($_POST['seller_id']) && !empty($_POST['seller_id']) && isset($_POST['transfer_fund_seller']) && !empty($_POST['transfer_fund_seller'])) {
    ...
    if ($fn->add_wallet_transaction('seller_wallet_transactions', $type, $amount, 0, 0, $id, $message)) {
...
if (isset($_POST['update_return_request']) && $_POST['update_return_request'] == 1) {
    ...
    if ($shipping_type == 1) {
        $res = $fn->get_data('return_requests', 'id=' . $id, $columns = ['status']);
        if ($res[0]['status'] == 1) {
            echo "<label class='alert alert-danger'>Return request already approved.</label>";
            return false;
        }
        if ($return_status == 1) {
            $res_order = $fn->get_data('orders', 'id=' . $id, $columns = ['final_total', 'total', 'user_id', 'payment_method', 'wallet_balance', 'delivery_charge', 'tax_amount', 'status', 'area_id']);
            ...
            $wallet_txn_id = $fn->add_wallet_transaction('wallet_transactions', 'credit', $total_amount, $id, $order_item_id, $user_id, 'Balance credited against item cancellation...');
            $user_data = $fn->get_data('users', 'id=' . $user_id, $columns = ['name', 'email', 'mobile', 'country_code']);
            ...
            $res_order_item = $fn->get_data('order_items', 'id=' . $order_item_id, $columns = ['active_status', 'status']);
            ...
            $db->update('order_items', 'id=' . $order_item_id, $data);
            // $item_data = array(
...
if (isset($_POST['manage_customer_wallet']) && isset($_POST['user_id'])) {
    ...
    $new_balance = $type == 'credit' ? $balance + $amount : $balance - $amount;
    $fn->update_wallet_balance($new_balance, $user_id, 'users');
    if ($fn->add_wallet_transaction('wallet_transactions', $type, $amount, $order_id = "", 0, $user_id, $message)) {
        echo "<label class='alert alert-success'>Balance Updated Successfully.</label>";
    } else {
        echo "<label class='alert alert-danger'>Some Error Occurred! Please Try Again.</label>";
    }
...
if (isset($_POST['update_withdrawal_request']) && $_POST['update_withdrawal_request'] == 1) {
    ...
    if ($status == 2) {
        if ($type1 == 'user') {
            ...
            $fn->add_wallet_transaction('wallet_transactions', 'credit', $amount, $order_id = "", 0, $type_id, 'Balance credited on withdrawal request cancellation.');
        }
        ...
        if ($type1 == 'seller') {
            ...
            $fn->add_wallet_transaction('seller_wallet_transactions', 'credit', $amount, $order_id = "", 0, $type_id, $message);
        }
    }
...
// update bulk product - update products in bulk using  a CSV file
...
if (isset($_POST['bulk_update']) && $_POST['bulk_update'] == 1) {
    ...
    $count1 = 0;
    $error = false;
    $filename = $_FILES["update_file"]["tmp_name"];
    // print_r($filename);

    $result = $fn->validate_image($_FILES["update_file"], false);
    ...
    if ($_FILES["update_file"]["size"] > 0 && $error == false) {
        ...
        while (($emapData = fgetcsv($file, 10000, ",")) !== FALSE) {
            if ($count != 0) {
                ...
                $emapData[12] = trim($db->escapeString($emapData[12])); // seller_id
                $emapData[13] = trim($db->escapeString($emapData[13])); // is_approved
                $emapData[14] = trim($db->escapeString($emapData[14])); // deliverable_type
                $emapData[15] = trim($db->escapeString($emapData[15])); // pincodes
                $emapData[16] = trim($db->escapeString($emapData[16])); // return_days
                $emapData[17] = trim($db->escapeString($emapData[17])); // tax_id
                $emapData[18] = trim($db->escapeString($emapData[18])); // standard_shipping
                $emapData[19] = trim($db->escapeString($emapData[19])); // pickup_location

                $emapData[20] = trim($db->escapeString($emapData[20])); // variant id//
                $emapData[21] = trim($db->escapeString($emapData[21])); // type
                $emapData[22] = trim($db->escapeString($emapData[22])); // Measurement
                $emapData[23] = trim($db->escapeString($emapData[23])); // Measurement Unit ID
                $emapData[24] = trim($db->escapeString($emapData[24])); // Price
                $emapData[25] = trim($db->escapeString($emapData[25])); // Discounted Price
                $emapData[26] = trim($db->escapeString($emapData[26])); // Serve For
                $emapData[27] = trim($db->escapeString($emapData[27])); // Stock
                $emapData[28] = trim($db->escapeString($emapData[28])); // Stock Unit ID
                $emapData[29] = trim($db->escapeString($emapData[29])); // weight
                $emapData[30] = trim($db->escapeString($emapData[30])); // height
                $emapData[31] = trim($db->escapeString($emapData[31])); // breadth
                $emapData[32] = trim($db->escapeString($emapData[32])); // length
                $emapData[33] = (!empty($emapData[33]) && $emapData[33] != "") ? trim($db->escapeString($emapData[33])) : ""; // hsn_code
                ...
                if (!empty($emapData[2])) {
                    $seller = $fn->get_data('seller', "id=" . $emapData[12], $columns = ['name']);
                    if (empty($seller)) {
                        echo '<p class="alert alert-danger">Seller is not exist check the seller_id at row - ' . $count . '</div>';
                        return false;
                    }
                    $sql = "SELECT categories FROM seller WHERE id = " . $emapData[12];
                    $db->sql($sql);
                    $res = $db->getResult();

                    if (strpos($res[0]['categories'], $emapData[2]) === false) {
                ...
                if (!empty($emapData[17])) {
                    $tax = $fn->get_data('taxes', "id = " . $emapData[17], ['id']);
                    if (empty($tax)) {
                ...
                if (empty($emapData[20])) {
                    echo '<p class="alert alert-danger">Varient ID  is empty or invalid at row - ' . $count . '</div>';
                    return false;
                }
                // add type index
                $index1 = 21;
                ...
                $sql = "SELECT * FROM product_variant WHERE id=" . $emapData[20];
                $db->sql($sql);
                $result = $db->getResult();
                if (empty($result)) {
        ...
        while (($emapData = fgetcsv($file, 10000, ",")) !== FALSE) {
            if ($count1 != 0) {
                ...
                $emapData[1] = trim($db->escapeString($emapData[1])); // product name
                $emapData[2] = trim($db->escapeString($emapData[2])); // category id
                $emapData[3] = !empty($emapData[3]) ? trim($db->escapeString($emapData[3])) : 0; // subcategory id
                $emapData[4] = trim($db->escapeString($emapData[4])); // indicator
                $emapData[5] = trim($db->escapeString($emapData[5])); // manufacturer
                $emapData[6] = trim($db->escapeString($emapData[6])); // made in
                $emapData[7] = !empty($emapData[7]) ? trim($db->escapeString($emapData[7])) : 0; // return status
                $emapData[8] = !empty($emapData[8]) ? trim($db->escapeString($emapData[8])) : 0; // cancel status
                $emapData[9] = trim($db->escapeString($emapData[9])); // till status
                $emapData[10] = trim($db->escapeString($emapData[10])); // description
                $emapData[11] = trim($db->escapeString($emapData[11])); // image
                $emapData[12] = trim($db->escapeString($emapData[12])); // seller_id
                $emapData[13] = (!empty($emapData[13]) && $emapData[13] != "") ? trim($db->escapeString($emapData[13])) : 0; // is_approved
                $emapData[14] = (!empty($emapData[14]) && $emapData[14] != "") ? trim($db->escapeString($emapData[14])) : ""; // deliverable_type
                $emapData[15] = (!empty($emapData[15]) && $emapData[15] != "") ? trim($db->escapeString($emapData[15])) : ""; // pincodes
                $emapData[16] = (!empty($emapData[16]) && $emapData[16] != "") ? trim($db->escapeString($emapData[16])) : "0"; // return_days
                $emapData[17] = (!empty($emapData[17]) && $emapData[17] != "") ? trim($db->escapeString($emapData[17])) : "0"; // tax_id
                $emapData[17] = (!empty($emapData[18]) && $emapData[18] != "") ? trim($db->escapeString($emapData[18])) : "0"; // standard_shipping 
                $emapData[18] = (!empty($emapData[19]) && $emapData[19] != "") ? trim($db->escapeString($emapData[19])) : "0"; // pickup_location

                $emapData[20] = trim($db->escapeString($emapData[20])); // product variant id//
                $emapData[21] = trim($db->escapeString($emapData[21])); // type
                $emapData[22] = trim($db->escapeString($emapData[22])); // Measurement
                $emapData[23] = trim($db->escapeString($emapData[23])); // Measurement Unit ID
                $emapData[24] = trim($db->escapeString($emapData[24])); // Price
                ...
                $slug = $function->slugify($emapData[1]);
                $data = array(
                    // 'id' => $emapData[0],
                    'name' => $emapData[1],
                    'slug' => $slug,
                    'category_id' => $emapData[2],
                    'subcategory_id' => $emapData[3],
                    'indicator' => $emapData[4],
                    'manufacturer' => $emapData[5],
                    'made_in' => $emapData[6],
                    'return_status' => $emapData[7],
                    'cancelable_status' => $emapData[8],
                    'till_status' => $emapData[9],
                    'description' => $emapData[10],
                    'image' => $emapData[11],
                    'seller_id' => $emapData[12],
                    'is_approved' => $emapData[13],
                    'type' => $emapData[21],
                    'pincodes' => $emapData[15],
                    'return_days' => $emapData[16],
                    'tax_id' => $emapData[17],
                    'standard_shipping' => $emapData[18],
                    'pickup_location' => $emapData[19],
                );

                // print_r($data);
                // print_r("*****");
                // echo "$emapData[1], $emapData[2],
                // $emapData[3], $emapData[4],
                // $emapData[5], $emapData[6],
                // $emapData[7], $emapData[8],
                // $emapData[9], $emapData[10],
                // $emapData[11], $emapData[12],
                // $emapData[13], $emapData[14],
                // $emapData[15], $emapData[16],$emapData[17]";
                // return false;
                $db->update('products', 'id=' . $emapData[0], $data);
                $res = $db->getResult();
                ...
                for ($i = 0; $i < $total_variants; $i++) {
                    ...
                    $variant_data[$i]['product_id'] = $emapData[0];
                    // $variant_data[$i]['id'] = $emapData[20];
                    // $index++;
                    ...
                    $index++;
                    $variant_data[$i]['hsn_code'] = $emapData[$index];
                    $index++;
                    // print_r($variant_data);
                    // return false;
                    $db->update('product_variant', 'id=' . $emapData[20], $variant_data[$i]);
                    $res1 = $db->getResult();
                }
            }
            $count1++;
    ...
    if ($_FILES["upload_file"]["size"] > 0 && $error == false) {
        ...
        while (($emapData = fgetcsv($file, 10000, ",")) !== FALSE) {
            ...
            if ($count != 0) {
                ...
                $emapData[31] = (!empty($emapData[31]) && $emapData[31] != "") ? trim($db->escapeString($emapData[31])) : ""; // hsn_code
                ...
                if (!empty($emapData[1])) {
                    $seller = $fn->get_data('seller', "id=" . $emapData[11], $columns = ['name']);
                    if (empty($seller)) {
                        echo '<p class="alert alert-danger">Seller is not exist check the seller_id at row - ' . $count . '</div>';
                        return false;
                    }
                ...
                if (!empty($emapData[16])) {
                    $tax = $fn->get_data('taxes', "id = " . $emapData[16], ['id']);
                    if (empty($tax)) {
                        echo '<p class="alert alert-danger">Tax ID  is invalid at row - ' . $count . '</div>';
                        return false;
                    }
        ...
        while (($emapData = fgetcsv($file, 10000, ",")) !== FALSE) {
            if ($count1 != 0) {
                $pr_approval = $fn->get_data('seller', "id = " . $emapData[12], ['require_products_approval']);
                $is_approved = isset($pr_approval[0]['require_products_approval']) && $pr_approval[0]['require_products_approval'] == 0 ? 1 : 0;
                $emapData[0] = trim($db->escapeString($emapData[0])); // product name
                $emapData[1] = trim($db->escapeString($emapData[1])); // category id
                ...
                for ($i = 0; $i < $total_variants; $i++) {
                    ...
                    //print_r($variant_data[$i]['product_id']);
                    $variant_data[$i]['type'] = $emapData[$index];
                    $index++;
                    $variant_data[$i]['measurement'] = $emapData[$index];
                    $index++;
...
if (isset($_POST['boy_id']) && isset($_POST['cash_collection']) && !empty($_POST['cash_collection'])) {
    ...
    $id = $db->escapeString($fn->xss_clean($_POST['boy_id']));
    $cash = $fn->get_data('delivery_boys', "id='" . $id . "'", $columns = ['cash_received']);
    if (isset($cash[0]['cash_received'])) {
        ...
        $db->sql($sql);
        $fn->add_transaction($amount, "", $id, 'delivery_boy_cash_collection', $message, $date);
...
if (isset($_POST['seller_id']) && isset($_POST['get_category_wise_commission']) && !empty($_POST['get_category_wise_commission'])) {
    ...
    if (isset($categories[0]['categories']) && !empty($categories[0]['categories'])) {
        ...
        for ($i = 0; $i < count($categories); $i++) {
            $category = $fn->get_data('category', "id='" . $categories[$i] . "'", $columns = ['id', 'name']);
            if (isset($category[0]['id'])) {
                $commission = $fn->get_data('seller_commission', "seller_id='" . $seller_id . "' and category_id='" . $category[0]['id'] . "'", $columns = ['commission']);
...
if (isset($_POST['seller_id']) && isset($_POST['save_seller_commission']) && !empty($_POST['save_seller_commission'])) {
    ...
    for ($i = 0; $i < count($category_ids); $i++) {
        $result = $fn->get_data('seller_commission', "seller_id='" . $seller_id . "' and category_id='" . $category_ids[$i] . "'", $columns = ['id']);
...
if (isset($_POST['shiprocket']) && isset($_POST['shiprocket_email']) && isset($_POST['shiprocket_password']) && isset($_POST['free_delivery_charge']) && isset($_POST['minimum_oredr_amount'])) {
    ...
    if (empty($data_shiprocket)) {
        ...
        $free_delivery_charge = $_POST['free_delivery_charge'];
        $minimum_oredr_amount = $_POST['minimum_oredr_amount'];
        ...
        $json_data = array('shiprocket' => $shiprocket, 'shiprocket_email' => $shiprocket_email, 'shiprocket_password' => $shiprocket_password, 'webhook_token' => $webhook_token, 'free_delivery_charge' => $free_delivery_charge, 'minimum_oredr_amount' => $minimum_oredr_amount);
    ...
    if ($_FILES["upload_file"]["size"] > 0 && $error == false) {
        ...
        while (($emapData = fgetcsv($file, 10000, ",")) !== FALSE) {
            if ($count1 != 0) {
                ...
                $sql = "INSERT INTO pincodes (`pincode`,`status`) VALUES ('" . $emapData[0] . "' , '" . $emapData[1] . "')";
                $db->sql($sql);
            }
            $count1++;
        }
    ...
    if ($_FILES["upload_file"]["size"] > 0 && $error == false) {
        ...
        while (($emapData = fgetcsv($file, 10000, ",")) !== FALSE) {
            if ($count != 0) {
                ...
                if (!empty($emapData[1])) {
                    $city = $fn->get_data('cities', "id=" . $emapData[1], $columns = ['name']);
                    if (empty($city)) {
                ...
                if (!empty($emapData[2])) {
                    $pincode = $fn->get_data('pincodes', "id=" . $emapData[2], $columns = ['pincode']);
                    if (empty($pincode)) {
