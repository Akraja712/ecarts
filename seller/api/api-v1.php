...
if (isset($_POST['get_products']) && !empty($_POST['get_products'] == 1)) {
    ...
    if (isset($_POST['category_id']) && !empty($_POST['category_id']) && is_numeric($_POST['category_id'])) {
        $seller_category = $fn->get_data('seller', "id=" . $seller_id, $columns = ["categories"]);
        ...
        $categories = $fn->get_data('seller', "id=" . $seller_id, $columns = ["categories"]);
    ...
    foreach ($res as $row) {
        $row['category_name'] = $fn->get_data('category', 'id=' . $res[$i]['category_id'], ['name'])[0]['name'];
        $row['subcategory_name'] = !empty($res[$i]['subcategory_id']) ? $fn->get_data('subcategory', 'id=' . $res[$i]['subcategory_id'], ['name'])[0]['name'] : "";
    ...
    if (isset($_POST['shipping_type']) && !empty($_POST['shipping_type'])) {
        ...
        if ($shipping_type == 'standard') {
            ...
            foreach ($res as $row) {
                ...
                $res_seller = $fn->get_data('seller', "id=" . $res[$i]['seller_id'], $columns = ['name', 'mobile', 'latitude', 'longitude', 'state', 'street', 'pincode_id', 'city_id']);
                $res_pincode = $fn->get_data('pincodes', "id=" . $res_seller[0]['pincode_id'], $columns = ['pincode']);
                $res_city = $fn->get_data('cities', "id=" . $res_seller[0]['city_id'], $columns = ['name']);
                ...
                for ($j = 0; $j < count($res[$i]['items']); $j++) {
                    ...
                    if (!empty($res[$i]['items'][$j]['seller_id'])) {
                        $seller_info = $fn->get_data('seller', "id=" . $res[$i]['items'][$j]['seller_id'], $columns = ['name', 'store_name']);
                    ...
                    if (!empty($res[$i]['items'][$j]['delivery_boy_id'])) {
                        $delvery_boy_info = $fn->get_data('delivery_boys', "id=" . $res[$i]['items'][$j]['delivery_boy_id'], $columns = ['name']);
                    ...
                    $order_tracking_data = [];
                    $order_tracking_data = $fn->get_data('order_trackings', 'order_item_id=' . $res[$i]['items'][$j]['id'], ['*']);
...
if (isset($_POST['update_order_status']) && !empty($_POST['update_order_status'] == 1)) {
    ...
    $sql = "select count(DISTINCT(order_id)) as total from order_items oi JOIN product_variant pv on pv.id=oi.product_variant_id JOIN products p on p.id=pv.product_id where p.standard_shipping=0 AND oi.id=$id and oi.id in ($order_item_ids) and oi.seller_id=" . $seller_id;
    $db->sql($sql);
    $res = $db->getResult();
    // print_r($res);
    $total_count = $res[0]['total'];
    ...
    foreach ($res as $row) {
        ...
        for ($j = 0; $j < count($res[$i]['items']); $j++) {
            ...
            if (!empty($delivery_boy_id)) {
                $delvery_boy_info = $fn->get_data('delivery_boys', "id=" . $delivery_boy_id, $columns = ['name']);
                $res[$i]['items'][$j]['delivery_boy_name'] = $delvery_boy_info[0]['name'];
            } else {
                $res[$i]['items'][$j]['delivery_boy_name'] = "";
            }
...
if (isset($_POST['add_products']) && !empty($_POST['add_products'])) {
    ...
    $seller_data = $fn->get_data('seller', 'id=' . $seller_id, $columns = ['require_products_approval']);
    ...
    if ($_POST['shipping_type'] == 'standard') {
        ...
        $check = $fn->get_data('pickup_locations', "pickup_location='$pickup_location'", ['*']);
        if (empty($check)) {
            $response['error'] = true;
    ...
    if ($db->sql($sql)) {
        $res_inner = $fn->get_data('products', 'seller_id=' . $seller_id, $columns = ['id'], '0', '1', 'id', 'DESC');
    ...
    $product_data = $fn->get_data('products', "slug='" . $slug . "'", '');
    ...
    $images = $fn->get_data('product_variant', 'product_id=' . $product_data[0]['id'], ['images']);

    $seller_data = $fn->get_data('seller', "id=" . $product_data[0]['seller_id'], $columns = ['name', 'status']);

    if (!empty($product_data[0]['tax_id'])) {
        $tax_data = $fn->get_data('taxes', "id=" . $product_data[0]['tax_id'], $columns = ['title', 'percentage']);
    }


    $variant_data = $fn->get_data('product_variant', 'product_id =' . $product_id, '', '', '', 'id', 'ASC');
    foreach ($variant_data as $vari_data) {
        for ($i = 0; $i < count($vari_data['id']); $i++) {
            $variant_img = $fn->get_data('product_variant', "id=" . $vari_data['id'], $columns = ['images']);
            ...
            $ms_unit_name = $fn->get_data('unit', "id=" . $vari_data['measurement_unit_id'], $columns = ['id,short_code']);
            $stock_unit_name = $fn->get_data('unit', "id=" . $vari_data['stock_unit_id'], $columns = ['id,short_code']);
    ...
    $res_data = array(
        ...
        "subcategory_name" => !empty($subcategory_id) ? $fn->get_data('subcategory', 'id=' . $subcategory_id, ['name'])[0]['name'] : "",
        "tax_id" => $tax_id,
        "category_id" => $category_id,
        "category_name" => !empty($category_id) ? $fn->get_data('category', 'id=' . $category_id, ['name'])[0]['name'] : "",
...
if (isset($_POST['update_products']) && !empty($_POST['update_products'])) {
    ...
    $res = $fn->get_data('products', 'id = ' . $id, $columns = ['slug']);
    $i = 1;
    foreach ($res as $row) {
        if ($slug == $row['slug']) {
            $slug = $slug . '-' . $i;
    ...
    $res1 = $fn->get_data('seller', 'id = ' . $seller_id, $columns = ['categories']);
    $category_ids = $res1[0]['categories'];

    $category_data = $fn->get_data('category', 'id IN (' . $category_ids . ')', $columns = ['id', 'name'], '', '', 'id', 'asc');

    $subcategory = $fn->get_data('subcategory', 'category_id IN (' . $category_ids . ')', '');

    $res = $fn->get_data('products', 'id = ' . $id, $columns = ['image', 'other_images']);
    foreach ($res as $row) {
    ...
    $is_approved = $fn->get_data('products', 'id=' . $_POST['id'], $columns = ['is_approved']);
    $is_approved = $is_approved[0]['is_approved'];
    $is_approved = ($pr_approval == 0) ? 1 : $is_approved;

    $sub_category_name = $fn->get_data('subcategory', "id= $subcategory_id AND category_id=$category_id", ['name']);
    ...
    /* get products data */
    $product_data = $fn->get_data('products', 'id=' . $id, '');

    /*get seller data */
    $seller_data = $fn->get_data('seller', "id=" . $product_data[0]['seller_id'], $columns = ['name', 'status']);

    /*get product variants data */
    $pr_variant_test = $fn->get_data("product_variant", "product_id=" . $id, $columns = ['id']);
    ...
    for ($i = 0; $i < count($measurement); $i++) {
        ...
        $vari_image = $fn->get_data('product_variant', 'id=' . $product_variant_id[$i], $columns = ['id', 'images']);
        ...
        if ($_POST['type'] == "packet") {
            ...
            if ($data['id'] == 0) {
                ...
                $db->update('product_variant', 'id=' . $data['id'], $data);
            }
            $res = $db->getResult();
    ...
    $variant_data = $fn->get_data('product_variant', 'product_id =' . $id, '');
    foreach ($variant_data as $vari_data) {
        for ($i = 0; $i < count($vari_data['id']); $i++) {
            $variant_img = $fn->get_data('product_variant', "id=" . $vari_data['id'], $columns = ['images']);
...
if (isset($_POST['get_customers']) && !empty($_POST['get_customers'])) {
    ...
    */
    ...
    if (!empty($res)) {
        ...
        $res = $db->getResult();
        $rows = array();
        $tempRow = array(); foreach ($res as $row) {
            $tempRow['id'] = $row['id'];
        ...
        if ($fn->is_user_or_dboy_exists($type, $type_id)) {
            ...
            if ($balance >= $amount) {
                ...
                if ($fn->debit_balance($type, $type_id, $new_balance)) {
                    ...
                    if ($type == 'seller') {
                        $fn->add_wallet_transaction('seller_wallet_transactions', $type, $amount, $order_id, $order_item_id, $type_id, $message);
                    }
...
if (isset($_POST['get_pickup_location']) || !empty($_POST['get_pickup_location'])) {
    ...
    $sql_query = "SELECT COUNT(id) as total FROM  pickup_locations WHERE   seller_id=" . $_POST['seller_id'] . "  LIMIT $limit OFFSET $offset";
    $db->sql($sql_query);
    $total = $db->getresult();

    $sql_query = "SELECT * FROM  pickup_locations WHERE  seller_id=" . $_POST['seller_id'] . "   LIMIT $limit OFFSET $offset";
    // echo $sql_query;
    $db->sql($sql_query);
    $data = $db->getresult();
...
if (isset($_POST['get_slug']) || !empty($_POST['get_slug'])) {
    ...
    $sql_query = "SELECT pickup_location FROM  pickup_locations where seller_id=$seller_id";
    // echo $sql_query;
    $db->sql($sql_query);
    $data = $db->getresult();
...
if (isset($_POST['create_shiprocket_order']) && !empty($_POST['create_shiprocket_order'])) {
    ...
    $order_details = $fn->get_data('order_trackings', "order_id=$order_id AND order_item_id in (" . $_POST['order_item_ids'] . ")", ['*']);
    ...
    $res = $fn->process_shiprocket($order_id, $seller_id, $pickup_location, $final_total, $weight, $height, $breadth, $length, $order_item_ids);
    if (isset($res['status_code']) == 1) {
        $order_item_ids = isset($_POST['order_item_ids']);
        $sql = "select count(DISTINCT(order_id)) as total from order_items oi LEFT JOIN product_variant pv on pv.id=oi.product_variant_id LEFT JOIN products p on p.id=pv.product_id where p.standard_shipping=1 AND oi.order_id=$order_id and oi.id in ($order_item_ids) and oi.seller_id=" . $seller_id;
        $db->sql($sql);
        $res = $db->getResult();
        $total_count = $res[0]['total'];
        $sql = "select DISTINCT o.id,oi.seller_id, o.*,(select name from users u where u.id=o.user_id) as user_name from orders o LEFT JOIN order_items oi ON o.id=oi.order_id LEFT JOIN product_variant pv on pv.id=oi.product_variant_id LEFT JOIN products p on p.id=pv.product_id where p.standard_shipping=1 and o.id=$order_id  and oi.seller_id=$seller_id";
...
if (isset($_POST['send_pickup_request']) && !empty($_POST['send_pickup_request'])) {
    if (isset($_POST['shipment_id']) && !empty($_POST['shipment_id'])) {
        ...
        $seller_id = $_POST['seller_id'];
        $get_order_id = $fn->get_data('order_trackings', 'pickup_status=0 And shipment_id=' . $shipment_id, ['shiprocket_order_id']);
        ...
        $order_details = $fn->get_data('order_trackings', "shipment_id=$shipment_id", ['order_id', 'order_item_id']);
        ...
        if ($res) {
            ...
            $result['error'] = false;
            $result['message'] = "Request For Pickup Sent Successfully";
            $result['data'] = $res;
        } else {
            $result['error'] = true;
...
if (isset($_POST['track_order']) && !empty($_POST['track_order'])) {
    ...
    */
    ...
    if (isset($_POST['shipment_id']) && $_POST['order_item_id'] == 0) {
        ...
        $tracking_data = $fn->get_data('order_items', 'id=' . $order_item_id, ['status', 'active_status']);
...
if (isset($_POST['cancel_order']) && !empty($_POST['cancel_order'])) {
    if (isset($_POST['shipment_id']) && !empty($_POST['shipment_id'])) {
        $shipment_id = $_POST['shipment_id'];
        $get_order_id = $fn->get_data('order_trackings', 'is_canceled=0 And shipment_id=' . $shipment_id, ['shiprocket_order_id']);
        ...
        $shiprocket_order_id = $get_order_id[0]['shiprocket_order_id'];
...
if (isset($_POST['generate_awb']) && !empty($_POST['generate_awb'])) {
    ...
    $get_order_id = $fn->get_data('order_trackings', '(awb_code="" or awb_code IS null ) And shipment_id=' . $shipment_id, ['shiprocket_order_id']);
    if (empty($get_order_id)) {
        $res['error'] = true;
    ...
    if ($res['error'] == false) {
        ...
        $response['error'] = true;
        $response['message'] = "AWB could not be generated. " . $res['message'];
    }
    print_r(json_encode($response));
}
...
*/
if (isset($_POST['cancel_shiprocket_order']) && !empty($_POST['cancel_shiprocket_order'])) {

    if (isset($_POST['shipment_id']) && !empty($_POST['shipment_id'])) {
        $oders_data = $fn->get_data('order_trackings', 'shipment_id=' . $_POST['shipment_id'], ['shiprocket_order_id']);
...
shipment_id : 12345678 
*/
if (isset($_POST['download_label']) && !empty($_POST['download_label'])) {

    if (isset($_POST['shipment_id']) && !empty($_POST['shipment_id'])) {
        $label = $fn->get_data('order_trackings', 'shipment_id=' . $_POST['shipment_id'], ['labels']);

        $label_url = json_decode($label[0]['labels'], true);

        if (!empty($label_url)) {
            $result['error'] = false;
            $result['message'] = "Your label download successfully";
            $result['data'] = $label_url['label_url'];
        } else {
            $result['error'] = true;
            $result['message'] = "Label not generated or Order cancelled";
        }
        print_r(json_encode($result));
    }
}
...
shipment_id : 12345678 
*/
if (isset($_POST['download_manifest']) && !empty($_POST['download_manifest'])) {

    if (isset($_POST['shipment_id']) && !empty($_POST['shipment_id'])) {

        $manifests = $fn->get_data('order_trackings', 'shipment_id=' . $_POST['shipment_id'], ['manifests']);
        $manifest_url = json_decode($manifests[0]['manifests'], true);

        if (!empty($manifest_url)) {
            $result['error'] = false;
            $result['message'] = "Your manifest download successfully";
            $result['data'] = $manifest_url['manifest_url'];
        } else {
            $result['error'] = true;
            $result['message'] = "Manifest not generated or Order cancelled";
        }
        print_r(json_encode($result));
    }
}
...
if (isset($_POST['generate_status']) && !empty($_POST['generate_status'])) {
    ...
    if (isset($_POST['shipment_id']) && !empty($_POST['shipment_id'])) {
        ...
        $result['manifest_generate_status'] = (isset($manifests[0]['manifests']) && !empty($manifests[0]['manifests'])) ? true : false;
        $result['label_generate_status'] = (isset($label[0]['labels']) && !empty($label[0]['labels'])) ? true : false;
...
if (isset($_POST['order_tracking']) && !empty($_POST['order_tracking'])) {

    if (isset($_POST['shipment_id']) && !empty($_POST['shipment_id'])) {

        $orders_data = $fn->get_data('order_trackings', 'shipment_id=' . $_POST['shipment_id'], ['*']);

        //  print_r($orders_data);
        if (!empty($orders_data)) {
            $res = $temp = array();
            foreach ($orders_data as $row) {
                $manifet_url = isset($row['manifests']) ? json_decode($row['manifests'], true) : "";
                $label_url = isset($row['labels']) ? json_decode($row['labels'], true) : "";

                $temp['id'] = $row['id'];
                $temp['order_id'] = $row['order_id'];
                $temp['order_item_id'] = $row['order_item_id'];
                $temp['shiprocket_order_id'] = isset($row['shiprocket_order_id']) ? $row['shiprocket_order_id'] : "";
                $temp['shipment_id'] = isset($row['shipment_id']) ? $row['shipment_id'] : "";
                $temp['courier_company_id'] = isset($row['courier_company_id']) ? $row['courier_company_id'] : "";
                $temp['awb_code'] = isset($row['awb_code']) ? $row['awb_code'] : "";
                $temp['tracking_url'] = isset($row['tracking_url']) ? $row['tracking_url'] : "";
                $temp['pickup_status'] = isset($row['pickup_status']) ? $row['pickup_status'] : "";
                $temp['pickup_scheduled_date'] = isset($row['pickup_scheduled_date']) ? $row['pickup_scheduled_date'] : "";
                $temp['pickup_token_number'] = isset($row['pickup_token_number']) ? $row['pickup_token_number'] : "";
                $temp['status'] = isset($row['status']) ? $row['status'] : "";
                $temp['others'] = isset($row['others']) ? $row['others'] : "";
                $temp['pickup_generated_date'] = isset($row['pickup_generated_date']) ? json_decode($row['pickup_generated_date'], true) : "";
                $temp['manifests'] = isset($row['manifests']) ? $manifet_url['manifest_url'] : "";
                $temp['labels'] = isset($row['labels']) ? $label_url['label_url'] : "";
                $temp['data'] = isset($row['data']) ? $row['data'] : "";
                $temp['date'] = isset($row['date']) ? $row['date'] : "";
                $temp['is_canceled'] = isset($row['is_canceled']) ? $row['is_canceled'] : "";
                $temp['updated_at'] = isset($row['updated_at']) ? $row['updated_at'] : "";
                $temp['created_at'] = isset($row['created_at']) ? $row['created_at'] : "";
