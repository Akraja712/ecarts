...
if (isset($_POST['add_seller']) && $_POST['add_seller'] == 1) {
    ...
    if ($db->sql($sql)) {
        ...
        $user_data = $fn->get_data('seller', 'id=' . $res[0]['id'], $columns = ['name', 'email', 'mobile']);
...
if (isset($_POST['update_seller']) && !empty($_POST['update_seller'])) {
    ...
    if (isset($_POST['old_password']) && $_POST['old_password'] != '') {
        ...
        $res = $fn->get_data('seller', "id=" . $id, $column = ['password']);
...
if (isset($_POST['bulk_upload']) && $_POST['bulk_upload'] == 1) {
    ...
    if ($_FILES["upload_file"]["size"] > 0 && $error == false) {
        ...
        while (($emapData = fgetcsv($file, 10000, ",")) !== FALSE) {
            ...
            if ($count != 0) {
                ...
                if (!empty($emapData[14])) {
                    $tax = $fn->get_data('taxes', "id = " . $emapData[14], ['id']);
                    if (empty($tax)) {
        ...
        fclose($file);
        $file = fopen($filename, "r");
        $pr_approval = $fn->get_data('seller', "id = " . $session_seller_id, ['require_products_approval']);
