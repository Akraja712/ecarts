...
$support_email = isset($settings['support_email']) ? $settings['support_email'] : "";
...
$shiprocket_data = $function->get_settings('shiprocket', true);
...
if (isset($_POST['place_order']) && isset($_POST['user_id']) && !empty($_POST['product_variant_id']) && !empty($_POST['place_order'])) {
    ...
    $Date = date('Y-m-d');
    $admin_delivery_charge = 0;

    /* free delivery charge calculation while stander shipping */

    if ($shipping_type == 'standard') {
        if (isset($shiprocket_data['free_delivery_charge']) && $shiprocket_data['free_delivery_charge'] == "1") {
            if ($final_total >= $shiprocket_data['minimum_oredr_amount']) {

                if ($_POST['payment_method'] == "COD") {
                    $delivery_charage_by_item = $function->get_delivery_charge($address_id, $total);
                    // print_r($delivery_charage_by_item);
                }
                $admin_delivery_charge = $function->get_delivery_charge($address_id, $total);
            }
        }
    }
    /**** ------------------------------------------------ ****/
    ...
    for ($i = 0; $i < count($item_arr); $i++) {
        $res = $function->get_data('product_variant', 'id=' . $item_arr[$i], $columns = ['id']);
        if (empty($res[0])) {
            $response['error'] = true;
    ...
    if (isset($total) && isset($settings['min_order_amount']) && $total < $settings['min_order_amount']) {
        $response['error'] = true;
    ...
    $sql = "INSERT INTO `orders`(`user_id`,`otp`,`mobile`,`order_note`, `total`, `delivery_charge`, `tax_amount`, `tax_percentage`, `wallet_balance`, `promo_code`,`promo_discount`, `final_total`, `payment_method`, `address`, `latitude`, `longitude`, `delivery_time`, `status`, `active_status`,`order_from`,`pincode_id`,`area_id`,`address_id`,`admin_delivery_charge`) VALUES ('$user_id','$otp_number','$mobile','$order_note','$total','$delivery_charge','$order_total_tax_amt','$order_total_tax_per','$walletvalue','$promo_code','$promo_discount', '$final_total','$payment_method','$address','$latitude','$longitude','$delivery_time','$order_status','$active_status','$order_from','$pincode_id','$area_id','$address_id','$admin_delivery_charge')";
    $db->sql($sql);
    ...
    if ($wallet_used == 'true') {
        ...
        $wallet_txn_id = $function->add_wallet_transaction('wallet_transactions', 'debit', $wallet_balance, $order_id, 0, $user_id, 'Used against Order Placement');
    }
    ...
    for ($i = 0; $i < count($item_details); $i++) {
        ...
        $hsn_code = $db->escapeString($item_details[$i]['hsn_code']);
        ...
        $sql = "INSERT INTO `order_items`(`user_id`, `order_id`,`product_name`,`variant_name`, `product_variant_id`, `quantity`, `price`, `discounted_price`,`tax_amount`,`tax_percentage`, `sub_total`, `status`, `active_status`,seller_id,`hsn_code`) VALUES ('$user_id','$neworder_id','$product_name','$variant_name','$product_variant_id','$quantity','$price','$discounted_price','$tax_amount', $tax_percentage,'$order_sub_total','$order_item_status','$active_status','$seller_id','$hsn_code')";
        $db->sql($sql);
        $res = $db->getResult();
        ...
        if ($type == 'packet') {
            ...
            if ($variant_qty[0]['stock'] <= 0) {
                ...
                $db->update("product_variant", "id=$product_variant_id", $data);
                $res = $db->getResult();
            }
        } elseif ($type == 'loose') {
    ...
    if ($db->update('orders', 'id=' . $order_id, $data)) {
        $res = $db->getResult();
        // print_r($res);
...
if (isset($_POST['update_order_status']) && isset($_POST['order_item_id']) && isset($_POST['order_id'])) {
    ...
    $res = $function->get_data('orders', 'id=' . $id, $columns = ['user_id', 'payment_method', 'wallet_balance', 'total', 'delivery_charge', 'tax_amount']);
    $res_order_item = $function->get_data('order_items', 'id=' . $order_item_id, $columns = ['active_status', 'status']);
    $delivery_boy_id = 0;
    ...
    if (isset($_POST['delivery_boy_id']) && !empty($_POST['delivery_boy_id']) && $_POST['delivery_boy_id'] != "") {
        ...
        $res_delivery_boy_id = $function->get_data('order_items', 'id=' . $order_item_id, $columns = ['active_status', 'status', 'delivery_boy_id']);
        if ($res_delivery_boy_id[0]['active_status'] == "awaiting_payment") {
            ...
            if (
                ($res_delivery_boy_id[0]['delivery_boy_id'] == 0)
                || ($res_delivery_boy_id[0]['delivery_boy_id'] != $delivery_boy_id && $res_delivery_boy_id[0]['active_status'] != 'cancelled')
            ) {
                $delivery_boy_name = $function->get_data('delivery_boys', 'id=' . $delivery_boy_id, $columns = ['name']);
        ...
        if (!empty($res)) {
            ...
            if ($postStatus == 'cancelled' || $postStatus == 'returned') {
                ...
                if ($postStatus == 'cancelled') {
                    ...
                    $return_days = $function->get_data('products', 'id=' . $product_details['product_id'], $columns = ['return_days']);
            ...
            if ($postStatus == 'delivered') {
                ...
                if ($res_boy[0]['delivery_boy_id'] != 0) {
                    if (strtolower($res_boy[0]['payment_method']) == 'cod') {
                        ...
                        $db->sql($sql);
                        $function->add_transaction($cash_received, $order_item_id, $res_boy[0]['delivery_boy_id'], 'delivery_boy_cash', 'Delivery boy collected COD');
                    }
                    ...
                    if ($res_boy[0]['total'] >= $config['min-refer-earn-order-amount']) {
                        if ($res_count[0]['total'] == 0) {
                            if ($res_user[0]['friends_code'] != '') {
                                ...
                                $res_data = $function->get_data('users', "id='" . $res[0]['user_id'] . "'", $columns = ['friends_code', 'name']);
                                $friend_user = $function->get_data('users', "referral_code='" . $res_data[0]['friends_code'] . "'", $columns = ['id']);
                                if (!empty($friend_user))
                                    $function->add_wallet_transaction('wallet_transactions', 'credit', floor($bonus_amount), $id, 0, $friend_user[0]['id'], 'Refer & Earn Bonus on first order by ' . ucwords($res_data[0]['name']));
            ...
            if ($postStatus == 'received') {
                ...
                $db->update('order_items', 'order_id=' . $id, $data);
                // $db->update('order_items', "received", 'order_id=' . $id);

                /* get order data */
                $user_id1 = $function->get_data('orders', 'id=' . $id, $columns = ['user_id', 'total', 'delivery_charge', 'discount', 'final_total', 'payment_method', 'address', 'otp']);

                /* get user data */
                $user_email = $function->get_data('users', 'id=' . $user_id1[0]['user_id'], $columns = ['email', 'name']);
                $subject = "Order received successfully";
                ...
                for ($i = 0; $i < count($item_details); $i++) {
                    ...
                    if (!empty($seller_id)) {
                        $store_details = $function->get_data('seller', 'id=' . $seller_id, $columns = ['email', 'store_name']);
                    }
                    $item_data1[] = array(
                        'name' => $item_details[$i]['name'],
                        'store_name' => $store_details[0]['store_name'],
                        'tax_amount' => $order_item[$i]['tax_amount'],
                        'tax_percentage' => $order_item[$i]['tax_percentage'],
                        'tax_title' => $item_details[$i]['tax_title'],
                        'unit' => $item_details[$i]['measurement'] . " " . $item_details[$i]['measurement_unit_name'],
                        'qty' => $order_item[$i]['quantity'],
                        'subtotal' => $order_item[$i]['sub_total']
                    );
                    if (!empty($seller_id)) {
                        $seller_subject = "New order placed for " . $store_details[0]['store_name'];
            ...
            if ($db->update('order_items', 'id=' . $order_item_id, $final_status)) {
                $response['error'] = false;
                if ($postStatus == 'cancelled') {
                ...
                if ($postStatus != 'received' && $postStatus != 'returned') {
                    $user_data = $function->get_data('users', 'id=' . $user_id, $columns = ['name', 'email', 'mobile', 'country_code']);
        ...
        for ($j = 0; $j < count($order_items); $j++) {
            ...
            if (isset($_POST['delivery_boy_id']) && !empty($_POST['delivery_boy_id']) && $_POST['delivery_boy_id'] != "") {
                ...
                if (isset($res_delivery_boy_id[0]['delivery_boy_id']) && !empty($res_delivery_boy_id[0]['delivery_boy_id'])) {
                    if ($postStatus != '') {
                        $delivery_boy_name = $function->get_data('delivery_boys', 'id=' . $res_delivery_boy_id[0]['delivery_boy_id'], $columns = ['name']);
                        if ($postStatus == 'delivered') {
                            $title_delivery_boy = "Order item with ID : #" . $order_item_id . "  has been delivered";
                            $message_delivery_boy = "Hello, Dear " . ucwords($delivery_boy_name[0]['name']) . ", your order has been delivered. ID : #" . $order_item_id . ". Please take a note of it.";
                        } else {
            ...
            if ($postStatus == 'delivered') {
                ...
                if ($res_boy[0]['delivery_boy_id'] != 0) {
                    ...
                    $res_bonus = $function->get_data('delivery_boys', 'id=' . $res_boy[0]['delivery_boy_id'], $columns = ['name', 'bonus']);
                    $sql = 'select name,bonus from delivery_boys where id = ' . $res_boy[0]['delivery_boy_id'];
                    $db->sql($sql);
                    $res_bonus = $db->getResult();
                    $reward = $res_boy[0]['sub_total'] / 100 * $res_bonus[0]['bonus'];
                    ...
                    if ($res_boy[0]['total'] >= $config['min-refer-earn-order-amount']) {
                        if ($res_count[0]['total'] == 0) {
                            if ($res_user[0]['friends_code'] != '') {
                                ...
                                $res_data = $function->get_data('users', "referral_code='" . $res_user[0]['referral_code'] . "'", $columns = ['friends_code', 'name']);
                                $friend_user = $function->get_data('users', "referral_code='" . $res_data[0]['friends_code'] . "'", $columns = ['id']);
                                if (!empty($friend_user))
                                    $function->add_wallet_transaction('wallet_transactions', 'credit', floor($bonus_amount), $res_boy[0]['order_id'], $order_item_id, $friend_user[0]['id'], 'Refer & Earn Bonus on first order by ' . ucwords($res_data[0]['name']));
            ...
            if ($postStatus == 'received') {
                ...
                // $db->update('order_items', $data, 'id=' . $order_item_id);
                $db->update('order_items', 'order_id=' . $order_id, $data);
                // $db->update('order_items', "received", 'order_id=' . $id);

                /* get order data */
                $user_id1 = $function->get_data('orders', 'id=' . $order_id, $columns = ['user_id', 'total', 'delivery_charge', 'discount', 'final_total', 'payment_method', 'address', 'otp']);
...
if (isset($_POST['test']) && $_POST['test'] == true) {
    $res = $function->get_order($_POST['id']);
    // print_r($res);
}
    ...
    if ($res[0]['delivery_boy_id'] == 0) {
        ...
        if (!empty($serviceability) && isset($serviceability['status']) && $serviceability['status'] == 200) {
            ...
            if (!empty($res) && isset($res['order_id']) && !empty($res['order_id'])) {
                ...
                $db->update("order_items", "id=$item_id", $data);
                $response['error'] = false;
    ...
    foreach ($res as $row) {
        ...
        for ($j = 0; $j < count($res[$i]['items']); $j++) {
            ...
            if (!empty($res[$i]['items'][$j]['seller_id'])) {
                $seller_info = $function->get_data('seller', "id=" . $res[$i]['items'][$j]['seller_id'], $columns = ['name', 'store_name']);
            ...
            $sql = "SELECT id from return_requests where product_variant_id = " . $res[$i]['items'][$j]['variant_id'] . " AND user_id = " . $res[$i]['items'][$j]['user_id'];
            $db->sql($sql);
...
if (isset($_POST['track_order']) && !empty($_POST['track_order'])) {
    ...
    */
    ...
    if (isset($_POST['shipment_id']) && $_POST['order_item_id'] == 0) {
        ...
        $tracking_data = $function->get_data('order_items', 'id=' . $order_item_id, ['status', 'active_status']);
