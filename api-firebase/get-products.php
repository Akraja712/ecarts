...
if (isset($_POST['get_all_products']) && $_POST['get_all_products'] == 1) {
    ...
    */
    ...
    $product_slug = (isset($_POST['product_slug']) && !empty($_POST['product_slug'])) ? $db->escapeString($fn->xss_clean($_POST['product_slug'])) : "";
    ...
    if (!empty($pincode)) {
        $is_pincode = $fn->get_data("pincodes", "pincode=" . $pincode, $column = ['pincode', 'id']);
        // print_r($is_pincode);
        if (empty($is_pincode)) {
            $response['error'] = true;
    ...
    if (isset($_POST['seller_slug']) && !empty($_POST['seller_slug'])) {
        ...
        if (isset($_POST['category_id']) && !empty($_POST['category_id']) && is_numeric($_POST['category_id'])) {
            $seller_category = $fn->get_data('seller', "slug='" . $seller_slug . "'", $columns = ["categories"]);
    ...
    if ($shipping_type == 'standard') {
        ...
        if (isset($_POST['product_slug']) && !empty($_POST['product_slug'])) {
            $product_slug = $_POST['product_slug'];

            $sql = "SELECT count(p.id) as total FROM `products` p LEFT JOIN `seller` s ON s.id=p.seller_id LEFT JOIN `category` c ON c.id=p.category_id WHERE p.is_approved = 1 AND p.status = 1 AND p.standard_shipping=1 AND p.slug = '$product_slug' AND s.status = 1 AND c.status = 1 AND (s.categories like CONCAT('%', p.category_id ,'%')) $where ";
        } elseif (isset($_POST['slug']) && !empty($_POST['slug'])) {
            $slug = $_POST['slug'];

            $sql = "SELECT count(p.id) as total FROM `products` p LEFT JOIN `seller` s ON s.id=p.seller_id LEFT JOIN `category` c ON c.id=p.category_id WHERE p.is_approved = 1 AND p.status = 1 AND p.standard_shipping=1 AND p.slug = '$slug' AND s.status = 1 AND c.status = 1 AND (s.categories like CONCAT('%', p.category_id ,'%')) $where ";
        } elseif (isset($_POST['category_slug']) && !empty($_POST['category_slug'])) {
            $cate_slug = $_POST['category_slug'];
            $sql = "SELECT * FROM category WHERE `slug`= '$cate_slug'";
            $db->sql($sql);
            $total1 = $db->getResult();
            $test = $total1[0]['id'];
            // print_r($test);
            ...
            $sql = "SELECT count(p.id) as total FROM `products` p LEFT JOIN `seller` s ON s.id=p.seller_id LEFT JOIN `category` c ON c.id=p.category_id WHERE p.is_approved = 1 AND p.status = 1 AND p.standard_shipping=1 AND p.category_id = $test AND s.status = 1 AND c.status = 1 AND (s.categories like CONCAT('%', p.category_id ,'%')) $where ";
        } else {
        ...
        if (isset($_POST['product_slug']) && !empty($_POST['product_slug'])) {
            ...
            $sql = "SELECT count(p.id) as total FROM `products` p LEFT JOIN `seller` s ON s.id=p.seller_id LEFT JOIN `category` c ON c.id=p.category_id WHERE p.is_approved = 1 AND p.status = 1 AND p.standard_shipping=0 AND p.slug = '$product_slug' AND s.status = 1 AND c.status = 1 AND (s.categories like CONCAT('%', p.category_id ,'%')) $where ";
        } elseif (isset($_POST['slug']) && !empty($_POST['slug'])) {
            $slug = $_POST['slug'];

            $sql = "SELECT count(p.id) as total FROM `products` p LEFT JOIN `seller` s ON s.id=p.seller_id LEFT JOIN `category` c ON c.id=p.category_id WHERE p.is_approved = 1 AND p.status = 1 AND p.standard_shipping=0 AND p.slug = '$slug' AND s.status = 1 AND c.status = 1 AND (s.categories like CONCAT('%', p.category_id ,'%')) $where ";
        } elseif (isset($_POST['category_slug']) && !empty($_POST['category_slug'])) {
            $cate_slug = $_POST['category_slug'];
            $sql = "SELECT * FROM category WHERE `slug`= '$cate_slug'";
            $db->sql($sql);
            ...
            $sql = "SELECT count(p.id) as total FROM `products` p LEFT JOIN `seller` s ON s.id=p.seller_id LEFT JOIN `category` c ON c.id=p.category_id WHERE p.is_approved = 1 AND p.status = 1 AND p.standard_shipping=0 AND p.category_id = $test AND s.status = 1 AND c.status = 1 AND (s.categories like CONCAT('%', p.category_id ,'%')) $where ";
        } else {
    ...
    if ($shipping_type == "standard") {
        ...
        if (isset($_POST['product_slug']) && !empty($_POST['product_slug'])) {
            ...
            $sql = "SELECT p.*,p.type as d_type, s.store_name as seller_name,s.slug as seller_slug,s.status as seller_status,(SELECT " . $price . " FROM product_variant pv WHERE pv.product_id=p.id) as price FROM `products` p JOIN `seller` s ON s.id=p.seller_id LEFT JOIN `category` c ON c.id=p.category_id WHERE p.is_approved = 1 AND p.standard_shipping=1 AND p.slug = '$product_slug' AND p.status = 1 AND s.status = 1 AND c.status = 1 AND (s.categories like CONCAT('%', p.category_id ,'%')) $where ORDER BY $sort $order LIMIT $offset,$limit ";
        } elseif (isset($_POST['slug']) && !empty($_POST['slug'])) {
            $slug = $_POST['slug'];

            $sql = "SELECT p.*,p.type as d_type, s.store_name as seller_name,s.slug as seller_slug,s.status as seller_status,(SELECT " . $price . " FROM product_variant pv WHERE pv.product_id=p.id) as price FROM `products` p JOIN `seller` s ON s.id=p.seller_id LEFT JOIN `category` c ON c.id=p.category_id WHERE p.is_approved = 1 AND p.standard_shipping=1 AND p.slug = '$slug' AND p.status = 1 AND s.status = 1 AND c.status = 1 AND (s.categories like CONCAT('%', p.category_id ,'%')) $where ORDER BY $sort $order LIMIT $offset,$limit ";
        } elseif (isset($_POST['category_slug']) && !empty($_POST['category_slug'])) {
            $cate_slug = $_POST['category_slug'];
            $sql = "SELECT * FROM category WHERE `slug`= '$cate_slug'";
            $db->sql($sql);
            $categories = $db->getResult();
            $test = $categories[0]['id'];
            // print_r($test);
            // $name = $categories[0]['name'];

            $sql = "SELECT p.*,p.type as d_type, s.store_name as seller_name,s.slug as seller_slug,s.status as seller_status,(SELECT " . $price . " FROM product_variant pv WHERE pv.product_id=p.id) as price FROM `products` p JOIN `seller` s ON s.id=p.seller_id LEFT JOIN `category` c ON c.id=p.category_id WHERE p.is_approved = 1 AND p.standard_shipping=1 AND p.category_id = $test AND p.status = 1 AND s.status = 1 AND c.status = 1 AND (s.categories like CONCAT('%', p.category_id ,'%')) $where ORDER BY $sort $order LIMIT $offset,$limit ";
        } else {
        ...
        if (isset($_POST['product_slug']) && !empty($_POST['product_slug'])) {
            ...
            $sql = "SELECT p.*,p.type as d_type, s.store_name as seller_name,s.slug as seller_slug,s.status as seller_status,(SELECT " . $price . " FROM product_variant pv WHERE pv.product_id=p.id) as price FROM `products` p JOIN `seller` s ON s.id=p.seller_id LEFT JOIN `category` c ON c.id=p.category_id WHERE p.is_approved = 1 AND p.slug = '$product_slug' AND p.standard_shipping=0 AND p.status = 1 AND s.status = 1 AND c.status = 1 AND (s.categories like CONCAT('%', p.category_id ,'%')) $where ORDER BY $sort $order LIMIT $offset,$limit ";
        } elseif (isset($_POST['slug']) && !empty($_POST['slug'])) {

            $slug = $_POST['slug'];

            $sql = "SELECT p.*,p.type as d_type, s.store_name as seller_name,s.slug as seller_slug,s.status as seller_status,(SELECT " . $price . " FROM product_variant pv WHERE pv.product_id=p.id) as price FROM `products` p JOIN `seller` s ON s.id=p.seller_id LEFT JOIN `category` c ON c.id=p.category_id WHERE p.is_approved = 1 AND p.slug = '$slug' AND p.standard_shipping=0 AND p.status = 1 AND s.status = 1 AND c.status = 1 AND (s.categories like CONCAT('%', p.category_id ,'%')) $where ORDER BY $sort $order LIMIT $offset,$limit ";
        } elseif (isset($_POST['category_slug']) && !empty($_POST['category_slug'])) {
            ...
            $sql = "SELECT p.*,p.type as d_type, s.store_name as seller_name,s.slug as seller_slug,s.status as seller_status,(SELECT " . $price . " FROM product_variant pv WHERE pv.product_id=p.id) as price FROM `products` p JOIN `seller` s ON s.id=p.seller_id LEFT JOIN `category` c ON c.id=p.category_id WHERE p.is_approved = 1 AND p.category_id = $test AND p.standard_shipping=0 AND p.status = 1 AND s.status = 1 AND c.status = 1 AND (s.categories like CONCAT('%', p.category_id ,'%')) $where ORDER BY $sort $order LIMIT $offset,$limit ";
        } else {
            $sql = "SELECT p.*,p.type as d_type, s.store_name as seller_name,s.slug as seller_slug,s.status as seller_status,(SELECT " . $price . " FROM product_variant pv WHERE pv.product_id=p.id) as price FROM `products` p JOIN `seller` s ON s.id=p.seller_id LEFT JOIN `category` c ON c.id=p.category_id WHERE p.is_approved = 1 AND p.standard_shipping=0 AND p.status = 1 AND s.status = 1 AND c.status = 1 AND (s.categories like CONCAT('%', p.category_id ,'%')) $where ORDER BY $sort $order LIMIT $offset,$limit ";
        }
    }
    ...
    foreach ($res as $row) {
        ...
        if (!empty($pincode) || $pincode != "") {
            ...
            // print_r($res_pincodes);
            $pincodes = isset($res_pincodes) ? implode(",", array_column($res_pincodes, "pincode")) : "";
...
if (isset($_POST['get_variants_offline']) && $_POST['get_variants_offline'] == 1 && isset($_POST['variant_ids']) && !empty($_POST['variant_ids'])) {
    ...
    */
    ...
    $where = "";
    if (isset($_POST['pincode']) && !empty($_POST['pincode'])) {
        $pincode = $fn->get_data("pincodes", "pincode=" . $pincode, $column = ['pincode', 'id']);
        if (empty($pincode)) {
            $response['error'] = true;
            $response['message'] = "Invalid Pincode passed.";
            print_r(json_encode($response));
...
if (isset($_POST['check_deliverability']) && $_POST['check_deliverability'] == 1) {
    ...
    */
    ...
    if (isset($_POST['slug']) && !empty($_POST['slug'])) {
        $res = $fn->get_data('products', "slug='" . $_POST['slug'] . "'", $x = ['id']);
        ...
        if ($result[0]['standard_shipping'] == 1) {
            ...
            if ((isset($_POST['pincode_id']) && !empty($_POST['pincode_id'])) or (isset($_POST['pincode']) && !empty($_POST['pincode']))) {
                ...
                if (isset($_POST['pincode_id']) && !empty($_POST['pincode_id'])) {
                    $pincode = $fn->get_data('pincodes', "id='" . $_POST['pincode_id'] . "'", $columns = ['id,pincode']);
                    if (empty($pincode)) {
                        $response['error'] = true;
            ...
            if (isset($_POST['pincode']) && !empty($_POST['pincode'])) {
                $res = $fn->get_data('pincodes', "pincode='" . $_POST['pincode'] . "'", $columns = ['id']);
                if (empty($res)) {
                    $response['error'] = true;
                    $response['message'] = "Invalid pincode passed.";
                    print_r(json_encode($response));
            ...
            if (isset($_POST['pincode_id']) && !empty($_POST['pincode_id'])) {
                $pincode = $fn->get_data('pincodes', "id='" . $_POST['pincode_id'] . "'", $columns = ['id']);
