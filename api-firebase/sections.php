...
if (isset($_POST['get-all-sections']) && $_POST['get-all-sections'] == 1) {
    ...
    */
    ...
    if (!empty($pincode_id)) {
        $is_pincode = $fn->get_data("pincodes", "pincode=" . $pincode_id, $column = ['pincode', 'id']);
        if (empty($is_pincode)) {
            $response['error'] = true;
    ...
    if (!empty($res)) {
        foreach ($res as $row) {
            ...
            if (!empty($pincode)) {
                $res_pincode = $fn->get_data('pincodes', "id=" . $pincode, $column = ['pincode']);
        ...
        if (!empty($product_ids)) {
            ...
            foreach ($result1 as $row) {
                ...
                if (isset($row['seller_id']) != null) {
                    $seller_info = $fn->get_data("seller", "id=" . $row['seller_id'], $column = ['store_name']);
