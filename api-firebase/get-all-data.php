...
foreach ($result as $row) {
    ...
    foreach ($res_section_images as $offer_row) {
        ...
        $name = "";
        $slug = "";
        if ($offer_row['type'] == 'category') {

            $sql = 'select `name`,`slug` from category where id = ' . $offer_row['type_id'] . ' order by id desc';
            $db->sql($sql);
            $result1 = $db->getResult();
        ...
        if ($offer_row['type'] == 'products') {

            $sql = 'select `name`,`slug` from products where id = ' . $offer_row['type_id'] . ' order by id desc';
            $db->sql($sql);
            $result1 = $db->getResult();
            $name = (!empty($result1[0]['name'])) ? $result1[0]['name'] : "";
            $slug = (!empty($result1[0]['slug'])) ? $result1[0]['slug'] : "";
        }
        $section_temp['type'] = $offer_row['type'];
        $section_temp['type_id'] = $offer_row['type_id'];
        $section_temp['name'] = $name;
        $section_temp['slug'] = $slug;
        ($offer_row['offer_type'] == "video") ?  $temp['video'] = DOMAIN_URL . $offer_row['image'] : $temp['video'] = "";
        ($offer_row['offer_type'] == "image") ?  $temp['image'] = DOMAIN_URL . $offer_row['image'] : $temp['image'] = "";
        $section_temp['image'] = !empty($offer_row['image']) && $offer_row['image'] != 'null' ? DOMAIN_URL . $offer_row['image'] : "";
        $section_temp['offer_image_url'] = !empty($offer_row['offer_image_url']) ? $offer_row['offer_image_url'] : "";
...
foreach ($res_offer_images as $row) {
    ...
    $temp['type'] = $row['type'];
    $temp['type_id'] = $row['type_id'];
    $temp['offer_type'] = $row['offer_type'];
    $temp['name'] = $name;
    $temp['slug'] = $slug;
    ($row['offer_type'] == "video") ?  $temp['video'] = DOMAIN_URL . $row['image'] : $temp['video'] = "";
    ($row['offer_type'] == "image") ?  $temp['image'] = DOMAIN_URL . $row['image'] : $temp['image'] = "";
...
foreach ($res_offer_images as $row) {
    ...
    $cate_temp['type'] = $row['type'];
    $cate_temp['type_id'] = $row['type_id'];
    $cate_temp['offer_type'] = $row['offer_type'];
    $cate_temp['name'] = $name;
    $cate_temp['slug'] = $slug;
    ($row['offer_type'] == "video") ?  $cate_temp['video'] = DOMAIN_URL . $row['image'] : $cate_temp['video'] = "";
    ($row['offer_type'] == "image") ?  $cate_temp['image'] = DOMAIN_URL . $row['image'] : $cate_temp['image'] = "";
    $cate_temp['offer_image_url'] = !empty($row['offer_image_url']) ? $row['offer_image_url'] : "";
...
} elseif (isset($_POST['pincode']) && $_POST['pincode'] != "") {
    // $sql_query = "select s.* from products p join seller s on s.id=p.seller_id WHERE s.status = 1 and p.status=1 AND ((p.type='included' and FIND_IN_SET('$pincode_id', p.pincodes)) or p.type = 'all') OR ((p.type='excluded' and NOT FIND_IN_SET('$pincode_id', p.pincodes))) GROUP by p.seller_id ";
    $sql_query = "SELECT * FROM `seller` s WHERE s.status = 1 AND s.pincode_id=" . $pincode_id;
} else {
