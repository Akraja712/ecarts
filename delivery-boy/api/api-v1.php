...
if (isset($_POST['get_orders_by_delivery_boy_id']) && !empty($_POST['get_orders_by_delivery_boy_id'])) {
    ...
    */
    ...
    $i = 0;
    foreach ($res as $row) {
        $res_dboy = $fn->get_data('delivery_boys', "id=" . $row['delivery_boy_id'], $columns = ['name']);
        $seller_address = $fn->get_seller_address($row['seller_id']);
        $res_seller = $fn->get_data('seller', "id=" . $row['seller_id'], $columns = ['name', 'mobile', 'latitude', 'longitude', 'state', 'street', 'pincode_id']);
        ...
        if ($fn->is_user_or_dboy_exists($type, $type_id)) {
            ...
            if ($balance >= $amount) {
                ...
                if ($fn->debit_balance($type, $type_id, $new_balance)) {
                    ...
                    if ($type == 'users') {
                        $fn->add_wallet_transaction('wallet_transactions', 'debit', $amount, $order_id = "", 0, $type_id, 'Balance debited against withdrawal request.', '1');
                    }
                    $new_balance = 0;
...
if (isset($_POST['get_orders']) && !empty($_POST['get_orders'])) {
    ...
    foreach ($res as $row) {
        ...
        $res_seller = $fn->get_data('seller', "id=" . $row['seller_id'], $columns = ['name', 'mobile', 'latitude', 'longitude', 'state', 'street', 'pincode_id', 'city_id']);
        ...
        for ($i = 0; $i < count($item_ids); $i++) {
            ...
            if (isset($res_delivery_boy_id[0]['delivery_boy_id']) && !empty($res_delivery_boy_id[0]['delivery_boy_id'])) {
                if ($postStatus != '') {
                    ...
                    $delivery_boy_name = $fn->get_data('delivery_boys', 'id=' . $res_delivery_boy_id[0]['delivery_boy_id'], $columns = ['name']);
            ...
            if ($postStatus == 'delivered') {
                ...
                if ($res_boy[0]['delivery_boy_id'] != 0) {
                    ...
                    if (strtolower($res_boy[0]['payment_method']) == 'cod') {
                        ...
                        $delivery_cash = $fn->add_transaction($cash_received, $item_ids[$i], $res_boy[0]['delivery_boy_id'], 'delivery_boy_cash', 'Delivery boy collected COD');
                    }

                    $res_bonus = $fn->get_data('delivery_boys', 'id=' . $res_boy[0]['delivery_boy_id'], $columns = ['name', 'bonus']);
                    ...
                    if ($res_boy[0]['total'] >= $config['min-refer-earn-order-amount']) {
                        if ($res_count[0]['total'] == 0) {
                            if ($res_user[0]['friends_code'] != '') {
                                ...
                                $res_data = $fn->get_data('users', "referral_code='" . $res_user[0]['referral_code'] . "'", $columns = ['friends_code', 'name']);
                                $friend_user = $fn->get_data('users', "referral_code='" . $res_data[0]['friends_code'] . "'", $columns = ['id']);

                                if (!empty($friend_user)) {
                                    $fn->add_wallet_transaction('wallet_transactions', 'credit', floor($bonus_amount), $res_boy[0]['order_id'], $item_ids[$i], $friend_user[0]['id'], 'Refer & Earn Bonus on first order by ' . ucwords($res_data[0]['name']));
                                }
            ...
            if ($postStatus == 'cancelled') {
                if ($order_item_cancelled == true) {
                    ...
                    $db->update('order_items', 'id=' . $item_ids[$i], $data);
                }
            }
            ...
            if ($postStatus == 'received') {
                ...
                /* get order data */
                $user_id1 = $fn->get_data('orders', 'id=' . $order_id, $columns = ['user_id', 'total', 'delivery_charge', 'discount', 'final_total', 'payment_method', 'address', 'otp']);

                /* get user data */
                $user_email = $fn->get_data('users', 'id=' . $user_id1[0]['user_id'], $columns = ['email', 'name']);
                ...
                for ($i = 0; $i < count($item_details); $i++) {
                    ...
                    if (!empty($seller_id)) {
                        $store_details = $fn->get_data('seller', 'id=' . $seller_id, $columns = ['email', 'store_name']);
                    }
            ...
            if ($db->update('order_items', 'id=' . $item_ids[$i], $final_status)) {
...
if (isset($_POST['update_bulk_order_items']) && !empty($_POST['update_bulk_order_items'])) {
    ...
    */
    ...
    for ($i = 0; $i < count($order_items); $i++) {
        ...
        $db->sql($sql);
        $result = $db->getResult();
        if (!empty($result[0]['delivery_boy_id'])) {
            $dboy_id = $result[0]['delivery_boy_id'];
        } else {
            $dboy_id = "";
        }
